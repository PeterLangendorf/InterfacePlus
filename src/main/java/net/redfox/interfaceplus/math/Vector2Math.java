...
public class Vector2Math {
  ...
  private static double getDistanceBetweenTwoPoints(Vector2 origin, Vector2 goal) {
    return Math.sqrt(
        Math.pow(goal.getX() - origin.getX(), 2)
            + Math.pow(goal.getY() - origin.getY(), 2));
  }
  ...
  public static double getDirectionToPoint(Vector2 origin, Vector2 goal) {
    return Math.atan(goal.getY() - origin.getY() / goal.getX() - origin.getX());
  }

  /**
   * Calculates x and y increments that need to be added to your current x and y position to move
   * speed units in radians direction
   *
   * @param radians The angle in radians that you are traveling
   * @param speed The speed at which you are going
   * @return A Vector2 containing the increments that should be added to your position to move
   *     speed units in radians direction
   */
  public static Vector2 getIncrementsBySpeed(double radians, double speed) {
    IO.println(Math.toDegrees(radians));
    return new Vector2(speed * Math.cos(radians), speed * Math.sin(radians));
  }

  /**
   * Calculates the increments required to be added to a position in order to move speed units
   * towards goal from origin
   *
